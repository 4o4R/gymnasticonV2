...
# The WSL2 host kernel's sync() can hang intermittently (wedging in D state
# while flushing every filesystem, incl. the 9p mounts), which also hangs
# qemu-emulated syncs. Neutralize it across the whole build so both images can
# be produced reliably:
#  1. Patch the container's debootstrap so its trailing sync cannot hang stage0.
#  2. Replace /bin/sync + /usr/bin/sync in the rootfs with no-ops so stage1+
#     package postinsts cannot hang.
#  3. Neutralize the sync in pi-gen's unmount_image() so export-image cannot hang.
# The appliance image does not depend on /bin/sync at runtime (systemd flushes
# on shutdown), so shipping a no-op is safe.

...
- **Dual adapters when available**: We probe `/sys/class/bluetooth`, bring up all adapters, and default to using the onboard/non-USB radio for bike scanning (central) and the first USB BLE dongle for server advertising (peripheral). This is the preferred Pi Zero/Zero W setup because it keeps the fragile onboard controller out of advertising duty. If two adapters are present, heart-rate rebroadcast is enabled automatically.
